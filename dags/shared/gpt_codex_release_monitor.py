"""
GPT Codex Release Monitor DAG.

Monitors GitHub releases for openai/codex and creates tracking
...
The DAG also auto-updates Codex CLI in the worker container when new
releases are detected. Codex is used as an alternative LLM provider
in the customclaw bot platform.
...
ISSUE_LABEL = "codex-release"
ISSUE_LABELS = ["automated", "codex-release"]
...
@dag(
    dag_id="gpt_codex_release_monitor",
    description=(
        "Monitors openai/codex GitHub releases and creates "
        "tracking issues on baekenough/oh-my-customcode for new releases. "
        "Also auto-updates Codex CLI in the worker container."
    ),
    schedule="@hourly",
...
def gpt_codex_release_monitor() -> None:
    """Orchestrate release monitoring for GPT Codex CLI."""
    ...
    def fetch_releases() -> list[dict]:
        """Fetch the most recent releases from openai/codex.
        ...
        try:
            ...
            raise AirflowFailException(
                f"Failed to parse JSON from {CODEX_RELEASES_URL}: {exc}"
            ) from exc
        ...
        if not releases:
            log.info("No releases found for openai/codex.")
            return []
        ...
        log.info(
            "Fetched %d releases from openai/codex. "
            "Latest: %s (%s)",
            len(result),
            result[0]["tag_name"],
    ...
    def filter_new_releases(releases: list[dict]) -> list[dict]:
        ...
        ``codex-release`` label. A release is considered "already
        ...
        try:
            ...
            for issue in existing_issues:
                ...
                # [Codex CLI v1.0.0] New release detected
    ...
    def create_issues(new_releases: list[dict]) -> list[dict]:
        ...
        for release in new_releases:
            ...
            issue_title = f"[Codex CLI {tag}] New release detected"
    ...
    def analyze_issues(created_issues: list[dict]) -> None:
        ...
        Codex release notes and posts a structured analysis focused on
        customclaw platform impact (Codex is used as an alternative LLM
        provider in the bot platform).
        ...
        for issue in created_issues:
            ...
            try:
                prompt = (
                    f"Analyze this Codex CLI {tag} release notes and provide a structured analysis:\n\n"
                    f"{truncated_body}\n\n"
                    "Respond in Korean. Format your analysis as:\n"
                    "## 영향도 분석\n"
                    "- customclaw 플랫폼에 미치는 영향을 분석 (Codex는 봇 플랫폼에서 대체 LLM 프로바이더로 사용됨)\n"
                    "- Codex CLI 인터페이스 변경이 oh-my-customcode의 codex-exec 스킬/에이전트에 미치는 영향 식별\n\n"
                    "## 주요 변경사항\n"
                    "- 중요한 변경사항을 카테고리별로 정리 (신규 기능, 버그 수정, 개선)\n\n"
                    "## 필요한 조치\n"
                    "- customclaw에서 업데이트가 필요한 부분 구체적으로 명시 (codex-exec 에이전트, 스킬 등)\n"
                    "- 긴급도 표시 (높음/중간/낮음)\n\n"
                    "## Breaking Changes\n"
                    "- 호환성 문제가 있는 변경사항 식별 (없으면 '없음')\n"
                    "- Codex CLI API/인터페이스 변경으로 인한 통합 코드 수정 필요 여부\n"
                )
                ...
                comment_body = (
                    ...
                    f"{analysis}\n\n"
                    "---\n"
                    "_이 분석은 `gpt_codex_release_monitor` DAG에 의해 자동 생성되었습니다._"
                )
    ...
    # ------------------------------------------------------------------
    # Task 5: Update Codex CLI in the worker container
    # ------------------------------------------------------------------

    @task()
    def update_codex(new_releases: list[dict]) -> str:
        """Update Codex CLI in the worker container if new releases exist.

        Runs ``npm install -g @openai/codex@latest`` inside the
        ``customclaw-worker-1`` Docker container to ensure the worker
        always runs the latest Codex CLI version.

        Args:
            new_releases: List of release dicts from ``filter_new_releases``.
                          If empty, no update is performed.

        Returns:
            A string describing the outcome (skipped or update result).
        """
        if not new_releases:
            return "No new releases, skipping update."

        latest_tag = new_releases[0]["tag_name"]
        log.info("Updating Codex CLI to %s...", latest_tag)
        ...
        if result.returncode != 0:
            log.error("Codex update failed: %s", result.stderr[:500])
            return f"Update failed: {result.stderr[:200]}"

        log.info("Codex CLI updated successfully: %s", result.stdout.strip()[-200:])
        return f"Updated to latest. Output: {result.stdout.strip()[-200:]}"
...
def _build_issue_body(
    *,
    ...
    lines = [
        f"# Codex CLI {tag}{prerelease_badge}",
        "",
    ...
    lines += [
        ...
        "## Action Items",
        "",
        "- [ ] Review release notes for impact on customclaw platform",
        "- [ ] Update codex-exec agent/skill if Codex CLI interface changed",
        "- [ ] Test Codex integration with current customclaw version",
        "- [ ] Verify worker container Codex CLI update succeeded",
        "",
        "---",
        "",
        "_This issue was created automatically by the "
        "`gpt_codex_release_monitor` Airflow DAG._",
    ]
...
# Instantiate the DAG
dag_instance = gpt_codex_release_monitor()
